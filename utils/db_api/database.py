...
class DatabaseManager:
    ...
    def append_user(self, data):
        ...
        try:
            self.cursor.execute(f"""
                INSERT INTO users (telegram_id, full_name, phone_number, gender, location, exact_location)
                VALUES (?,?,?,?,?,?)
            """, (user_id, full_name, phone_number, gender, location, exact_location))
            self.conn.commit()
            return True
    ...
    def append_product(self, data):
        ...
        try:
            self.cursor.execute(f"""
                INSERT INTO product (name, price, description, photo, quantity)
                VALUES (?,?,?,?,?)
    ...
    def search_sticker_by_name(self, product):
        return self.cursor.execute(f"SELECT * FROM product WHERE name='{product}'").fetchall()
